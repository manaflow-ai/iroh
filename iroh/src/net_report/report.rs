...
impl Report {
    ...
    /// answered.
    #[cfg(not(wasm_browser))]
    pub(super) fn apply_qad_result(&mut self, family: AddrFamily, report: &QadProbeReport) {
        match family {
            AddrFamily::V4 => self.apply_qad_result_v4(report),
            AddrFamily::V6 => self.apply_qad_result_v6(report),
        }
    }
    ...
    #[cfg(not(wasm_browser))]
    pub(super) fn apply_qad_observation(&mut self, family: AddrFamily, report: &QadProbeReport) {
        match family {
            AddrFamily::V4 => self.apply_qad_observation_v4(report),
            AddrFamily::V6 => self.apply_qad_observation_v6(report),
        }
    }
    ...
    #[cfg(not(wasm_browser))]
    fn apply_qad_result_v4(&mut self, report: &QadProbeReport) {
        self.relay_latency
    ...
    #[cfg(not(wasm_browser))]
    fn apply_qad_result_v6(&mut self, report: &QadProbeReport) {
        self.relay_latency
    ...
    #[cfg(not(wasm_browser))]
    fn apply_qad_observation_v4(&mut self, report: &QadProbeReport) {
        self.relay_latency
            .update_relay(report.relay_url.clone(), report.latency, Probe::QadIpv4);
        let SocketAddr::V4(ipp) = report.addr else {
            warn!("received IPv6 address from IPv4 QAD: {}", report.addr);
    ...
    #[cfg(not(wasm_browser))]
    fn apply_qad_observation_v6(&mut self, report: &QadProbeReport) {
        self.relay_latency
            .update_relay(report.relay_url.clone(), report.latency, Probe::QadIpv6);
        let SocketAddr::V6(ipp) = report.addr else {
            warn!("received IPv4 address from IPv6 QAD: {}", report.addr);
