...
pub(super) struct QadConn {
    ...
    /// The most recent address the relay reported. Set to the first address,
    /// then updated by the actor as observations arrive. A later cycle reuses
    /// it instead of probing this family again.
    pub(super) last: QadProbeReport,
...
impl QadConns {
    ...
    pub(super) fn current(&self, family: AddrFamily) -> Option<QadProbeReport> {
        self.slot(family).map(|(_, c)| c.last.clone())
    }
...
/// address changes and forwards each one to `events` as a
/// [`ProbeEvent::QadObservation`], so a later change reaches the actor
